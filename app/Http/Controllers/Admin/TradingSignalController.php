...
const logTone = value => {
    if (value === 'Profit') return {
        ...
        ring: 'group-hover:border-emerald-100 group-hover:shadow-[0_4px_16px_rgba(16,185,129,0.08)]',
        accentBar: 'bg-emerald-400',
    }
    if (value === 'Loss') return {
        badge: 'bg-rose-100 text-rose-700',
        icon: 'bg-rose-50 text-rose-600 group-hover:bg-rose-100',
        ring: 'group-hover:border-rose-100 group-hover:shadow-[0_4px_16px_rgba(244,63,94,0.08)]',
        accentBar: 'bg-rose-400',
    }
    return {
        badge: 'bg-amber-100 text-amber-700',
        icon: 'bg-amber-50 text-amber-600 group-hover:bg-amber-100',
        ring: 'group-hover:border-amber-100 group-hover:shadow-[0_4px_16px_rgba(245,158,11,0.08)]',
    ...
    <div class="mx-auto w-full max-w-[1180px] space-y-6">
        ...
        <section class="grid grid-cols-1 gap-6 xl:grid-cols-[minmax(0,0.78fr)_minmax(0,1.22fr)]">
            ...
            <div class="overflow-hidden rounded-[22px] border border-slate-200 bg-white shadow-[0_1px_2px_rgba(15,23,42,0.04),0_12px_34px_rgba(15,23,42,0.06)]">
                ...
                <div class="market-logs-scroll relative max-h-[440px] overflow-y-auto p-3 sm:p-4">
                    <div class="relative space-y-2">
                        <div
                            v-if="logs.length > 1"
                            class="pointer-events-none absolute bottom-6 left-[29px] top-6 w-px bg-slate-100"
                        ></div>

                        <div
                            v-for="(log, index) in logs"
                            :key="`${log.time}-${index}`"
                            class="group relative flex gap-3 rounded-2xl px-2 py-1.5 transition hover:bg-slate-50"
                        >
                            <span
                                class="relative z-10 mt-1.5 flex h-9 w-9 shrink-0 items-center justify-center rounded-full ring-[3px] ring-white transition"
                            ...
                            <div
                                class="relative min-w-0 flex-1 overflow-hidden rounded-2xl border border-slate-100 bg-white shadow-[0_1px_2px_rgba(15,23,42,0.03)] transition"
                                :class="logTone(log.result).ring"
                            >
                                <span
                                    class="absolute inset-y-0 left-0 w-[3px]"
                                    :class="logTone(log.result).accentBar"
                                ></span>

                                <div class="flex flex-wrap items-center justify-between gap-2 px-4 pb-2 pt-3.5">
                                    <div class="flex min-w-0 flex-wrap items-baseline gap-x-2 gap-y-1">
                                        <span class="rounded-full px-2.5 py-0.5 text-[11px] font-bold uppercase tracking-wide" :class="logTone(log.result).badge">
                                            {{ log.result }}
                                        </span>
                                        <span class="text-[14px] font-semibold text-slate-900">{{ log.signal_type }}</span>
                                        <span class="text-[13px] text-slate-400">hit</span>
                                        <span class="text-[13px] font-medium text-slate-600">{{ log.hit_level }}</span>
                                    </div>

                                    <span
                                        class="shrink-0 whitespace-nowrap text-[11px] font-medium text-slate-400"
                                        :title="log.time"
                                    >
                                        {{ log.time }}
                                    </span>
                                ...
                                <div class="flex items-center gap-1.5 px-4 pb-3.5 text-[13px]">
                                    <span class="font-medium text-slate-400">Execution</span>
                                    <span class="font-bold tabular-nums text-slate-900">{{ log.price }}</span>
                                </div>
                            </div>
                        </div>
                    </div>
